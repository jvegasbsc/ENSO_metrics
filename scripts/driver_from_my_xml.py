...
def find_xml(
    name, frequency, variable, project="", experiment="", ensemble="", realm=""
):
    list_obs = list(ReferenceObservations().keys())
    if name in list_obs:
        file_name, file_area, file_land = find_xml_obs(
            name, frequency, variable
        )
    else:
...
def find_xml_cmip(
    model, project, experiment, ensemble, frequency, realm, variable
):
    file_name = join_path(
        xmldir,
        str(model)
    ...
    if variable not in listvar1:
        ...
        if variable not in listvar2:
            ...
            exit("")
        file_area, file_land = find_xml_fx(
            model, project=project, experiment=experiment, realm=new_realm
        )
    else:
        file_area, file_land = find_xml_fx(
            model, project=project, experiment=experiment, realm=realm
        )
    return file_name, file_area, file_land
...
def find_xml_fx(name, project="", experiment="", realm=""):
    ...
    if name in list_obs:
        file_area = join_path(
            xmldir, "obs_" + str(name) + "_glob_fx_O_areacell.xml"
        )
        file_land = join_path(
            xmldir, "obs_" + str(name) + "_glob_fx_O_landmask.xml"
        )
    else:
        file_area = join_path(
            xmldir,
        ...
        try:
            ...
            if isinstance(var_in_file, list):
                ...
                for var1 in var_in_file:
                    file_name, file_areacell, file_landmask = find_xml(
                        obs, frequency, var1
                    )
...
for mod in list_models:
    ...
    dict_metric[mod], _ = compute_collection(
        mc_name, dictDatasets, mod, netcdf=True, netcdf_name=netcdf, debug=True
    )
    tmp = sorted(
        list(dict_metric[mod]["value"].keys()), key=lambda v: v.upper()
    )
    for kk in tmp:
        print(
            kk.ljust(13) + ": " + str(dict_metric[mod]["value"][kk]["metric"])
        )
