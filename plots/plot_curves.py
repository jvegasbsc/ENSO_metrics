...
def create_minmax_plot(tab):
    ...
    list1 = [
        round(base * 4, 1) if base < 1 else int(round(base * 4, 0))
    ...
    if abs(locmini) == locmaxi:
        mini_out = -((interval / 2.0) + base)
        maxi_out = (interval / 2.0) + base
        for ii in range(3):
            if mini_out < locmini and mini_out + base < locmini:
                mini_out += base
                maxi_out -= base
    else:
        tmp_middle = locmini + (locmaxi - locmini) / 2.0
        ...
        print("middle = " + str(tmp_middle))
        if mini >= 0:
            mini_out = max([0, tmp_middle - interval / 2.0 - base])
        else:
            mini_out = tmp_middle - interval / 2.0 - base
        for ii in range(4):
            if mini_out > locmini:
                mini_out -= base
        for ii in range(4):
        ...
        if mini >= 0:
            if mini_out == 0:
                ...
                maxi_out = tmp_middle + interval / 2.0 + base
        else:
            maxi_out = min([0, tmp_middle + interval / 2.0 + base])
        print(maxi_out)
        for ii in range(4):
            if maxi_out < locmaxi:
                maxi_out += base
    ...
    if mini >= 0 and maxi < 40 and mult > 1:
        ...
        mult = mult / 10
    if (
        (mini >= 0 and maxi > 1 and maxi < 10) or mini > -5 and maxi < 5
    ) and mult < 1:
        mini_out = mini_out / 10
        maxi_out = maxi_out / 10
        mult = mult * 10
...
def plot_curves(
    ...
    if (
        axisname == "month" or axisname == "months" or axisname == "time"
    ) and len(tab_mod) == 72:
    ...
    minmax, mult = create_minmax_plot(
        [tab_mod.min(), tab_mod.max(), tab_obs.min(), tab_obs.max()]
    )
    if mult != 1.0:
    ...
    # plot curves
    ax.plot(
        axis, list(tab_mod), lw=4, label="model", color=ref_colors["model"]
    )
    ...
    for file1 in files_nc:
        ...
        if "SstDiv" in metric or "SstDur" in metric:
            ...
            var_in = sorted(
                [
                    var
                    for var in list(ff.keys())
                    if var not in no_var and ff[var].ndim == 1
                ],
                key=lambda v: v.upper(),
            )
            ...
            for nbr, var in enumerate(var_to_read):
                ...
                if varobs in var_in:
                    tab2 = ff[varobs]
                    metval = data_json["value"][metric]["metric"][
                        "ref_" + obsname
                    ]["value"]
                else:
                    tab2, metval = read_obs(
                        ff,
                        var_in,
                        met_var,
                        var,
                        data_json["value"][metric]["metric"],
                    )
                # plot
                metuni = data_json["metadata"]["metrics"][metric]["metric"][
                    "units"
                ].replace("C", "$^\circ$C")
                if "Corr" in metric:
                    mytext = "CORR"
                ...
                yname = yname + " (" + units + ")"
                name_png = OSpath__join(
                    path_plot,
                    metric + "_" + model + "_" + str(nbr + 1).zfill(2),
                )
                plot_curves(
                    tab1,
                    tab2,
