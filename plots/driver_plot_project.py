...
for proj in list_project:
    lpath = OSpath__join(
        path_in, proj + "/" + experiment + "/" + metric_collection
    )
    lname = (
        proj + "_" + experiment + "_" + metric_collection + "_v2019????.json"
    )
...
for proj in list_project:
    lpath = deepcopy(
        path_in
    )  # OSpath__join(path_in, proj + "/" + experiment + "/" + metric_collection)
...
# loop on metrics
list_metrics = sorted(
    list(defCollection(metric_collection)["metrics_list"].keys()),
...
for met in list_metrics:
    ...
    if big_ensemble is False:
        ...
        for proj in list_project:
            ...
            list_models = sorted(
                list(data_json[proj].keys()), key=lambda v: v.upper()
            )
            ...
            for mod in list_models:
                ...
                try:
                    ...
                    if metric_collection == "ENSO_tel" and "Map" in met:
                        ...
                        dict1[mod] = data_json[proj][mod]["value"][met][
                            "diagnostic"
                        ][mod]["value"]
            ...
            if proj == list_project[0]:
                if metric_collection == "ENSO_tel" and "Map" in met:
                    ...
                    tab = dict(
                        (
                            key1,
                            data_json[proj][list_models[0]]["value"][met][
                                "diagnostic"
                            ][key1]["value"],
                        )
                        for key1 in list(
            ...
            if metric_collection == "ENSO_tel" and "Map" in met:
                dicttmp1 = dict()
                for key1 in list(
                    data_json[proj][list_models[0]]["value"][met + "Corr"][
                        "metric"
                    ].keys()
                ):
                    dicttmp2 = dict()
                    for mod in my_data["models"][proj]:
                        dicttmp2[mod] = [
                            data_json[proj][mod]["value"][met + suffix][
                                "metric"
                            ][key1]["value"]
                            if key1
                            in list(
                ...
                my_data["metric_values"][proj] = dict(
                    (
                        key1,
                        dict(
                            (
                                mod,
                                alt_json[proj][mod]["value"][met]["metric"][
                                    key1
                                ]["value"]
                                if key1
                                in list(
                                    alt_json[proj][mod]["value"][met][
                                        "metric"
                                    ].keys()
                                )
                                else None,
                    ...
                    for key1 in list(
                        alt_json[proj][list_models[0]]["value"][met][
                            "metric"
                        ].keys()
                    )
                )
            ...
            print(
                ...
                + proj
                + " "
                + str(len(tab1)).zfill(2)
                + " file(s) "
                + str(len(tab2)).zfill(2)
                + " name(s)"
            )
            # print tab1
            # print list_models
            ...
            for proj in list_project:
                ...
                if len(list_models) > 0:
                    if metric_collection == "ENSO_tel" and "Map" in met:
                        for suffix in ["Corr", "Rmse"]:
                            for key1 in list(
                                data_json[proj][list_models[0]]["value"][
                                    met + suffix
                                ]["metric"].keys()
                            ):
                                try:
    ...
    if metric_collection == "ENSO_tel" and "Map" in met:
        ...
        units = data_json[proj][list(data_json[proj].keys())[0]]["metadata"][
            "metrics"
        ][met]["diagnostic"]["units"]
    my_data[
        "diagnostic_units"
    ] = units  # .replace("C", "$^\circ$C").replace("long", "$^\circ$long")
    if metric_collection == "ENSO_tel" and "Map" in met:
        units = [
            data_json[proj][list(data_json[proj].keys())[0]]["metadata"][
                "metrics"
            ][met + suffix]["metric"]["units"]
            for suffix in ["Corr", "Rmse"]
        ]
    ...
    if big_ensemble is True:
        main_plotter(
            ...
            my_data["diagnostic_units"],
            my_data["metric_values"],
            my_data["metric_units"],
            path_png=path_out,
            models2=my_data["models"],
            shading=True,
        )
    else:
        main_plotter(
