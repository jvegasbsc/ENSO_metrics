...
def multiportraitplot(
    ...
    nbr_space = 2
    nbrc = (
        sum([len(tab[ii][0]) for ii in range(len(tab))])
        + (len(tab) - 1) * nbr_space
    )
    ...
    for kk, tmp in enumerate(tab):
        ...
        try:
            ax.set_ylabel(ylabel[kk], fontsize=40, va="top")
        except:
            ax.set_ylabel(ylabel, fontsize=40, va="top")
    ...
    cax.annotate(
        ...
        va="bottom",
        arrowprops=dict(
            facecolor="k", width=8, headwidth=40, headlength=40, shrink=0.0
        ),
    )
    cax.text(
        5.2,
...
for mc in metric_collections:
    ...
    figure_name = OSpath__join(
        path_out, "portraitplot_" + str(mc) + "_modified_v1"
    )
...
if " ":
    ...
    figure_name = OSpath__join(
        ...
        + "metric_collections_v1",
    )
    tmp_num = ["a) ", "b) ", "c) "]
    title = [
        tmp_num[ii] + dict_mc[mc] for ii, mc in enumerate(metric_collections)
    ]
    # title = ""
    xlabel = ""
    ylabel = ""
