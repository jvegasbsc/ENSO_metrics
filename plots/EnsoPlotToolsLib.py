...
observations = sorted(
    list(ReferenceObservations().keys()), key=lambda v: v.upper()
)
...
def create_labels(label_name, label_ticks):
    if label_name == "months":
        ...
        if (
            min(label_ticks) < 0
            and max(label_ticks) > 0
            and 0 not in label_ticks
        ):
            label_ticks = NUMPYarray(label_ticks)
            while 0 not in label_ticks:
                label_ticks = label_ticks + 1
        label = [
            str(abs(int(ii))) + "$^\circ$S"
            if ii < 0
            else (str(abs(int(ii))) + "$^\circ$N" if ii > 0 else "eq")
        ...
        if (
            min(label_ticks) < 180
            and max(label_ticks) > 180
            and 180 not in label_ticks
        ):
            label_ticks = NUMPYarray(label_ticks)
...
def create_levels(labelbar):
    ...
    return [
        round(kk + jj * delta, 2) for kk in labelbar[:-1] for jj in range(mult)
    ] + [labelbar[-1]]
...
def minmax_plot(tab, metric=False):
    ...
    locmini, locmaxi = (
        int(MATHfloor(float(locmini) / mult)),
        int(MATHceil(float(locmaxi) / mult)),
    )
    ...
    if abs(locmini) == locmaxi:
        ...
        if locmini < 0 and locmaxi <= 0:
            ...
            sign = 1
        half_int = int(round(interval / 2.0))
        ...
        minmax = list(
            NUMPYaround(
                NUMPYarray([mini_out, maxi_out]) * sign, decimals=0
            ).astype(int)
        )
        ...
        if metric is True:
            if maxi_out < scalemaxi + base * 0.4:
                maxi_out += base
    tick_labels = NUMPYarange(mini_out, maxi_out + base / 2.0, base)
...
def read_diag(
    dict_diag, dict_metric, model, reference, metric_variables, shading=False
):
        ...
        if shading is True:
            diag_mod = [
                [
                    dict_diag[mod][mm]
                    for mm in sorted(
                        list(dict_diag[mod].keys()), key=lambda v: v.upper()
                    )
                ]
                for mod in model
            ]
    ...
    if isinstance(model, str):
        ...
        if shading is True:
            metric_value = [
                my_average(
                    [
                        dict_metric[mod][obs][mm]
                        for mm in sorted(
                            list(dict_metric[mod][obs].keys()),
...
def read_obs(
    xml, variables_in_xml, metric_variables, varname, dict_metric, model
):
    if len(metric_variables) == 1:
        for obs in observations:
            newvar = varname + obs
...
def reader(
    ...
    variables_in_file = sorted(
        [var for var in list(ff.keys())], key=lambda v: v.upper()
    )
    # read model
    tab_mod = list()
...
def read_var(
    ...
    if isinstance(filename_nc, str):
        tab_mod, tab_obs, metval, obs = reader(
            filename_nc,
            model,
            reference,
            var_to_read,
...
def shading_levels(tab, lev=[5, 25, 75, 95], axis=None):
    return [
        SCIPYstats__scoreatpercentile(tab, ll, axis=axis) for ll in lev
    ] + [my_average(tab, axis=axis)]
