...
dict_mc = {
    "ENSO_perf": "ENSO performance",
    "ENSO_proc": "ENSO processes",
    "ENSO_tel": "ENSO teleconnections",
}
...
def multiportraitplot(
    ...
    for kk, tmp in enumerate(tab):
        ...
        try:
            ax.set_title(title[kk], fontsize=40, y=1 + dy, loc="left")
        except:
            ax.set_title(title, fontsize=40, y=1 + dy, loc="left")
        # x axis
        ticks = [ii + 0.5 for ii in range(len(x_names))]
        ax.set_xticks(ticks)
        if kk != len(tab) - 1:
            ...
            for tick in ax.get_xticklabels():
                tick.set_rotation(90)
        try:
            ax.set_xlabel(xlabel[kk], y=dy, fontsize=40)
        except:
            ax.set_xlabel(xlabel, y=dy, fontsize=40)
        # y axis
        ...
        dx = 0.5 / (xx2 - xx1)
        try:
            ax.set_ylabel(ylabel[kk], fontsize=40, verticalalignment="top")
        except:
            ax.set_ylabel(ylabel, fontsize=40, verticalalignment="top")
    ...
    cbar.ax.set_yticklabels(
        ["-2 $\sigma$", "-1", "MMM", "1", "2 $\sigma$"], fontdict=fontdict
    )
    cax.annotate(
        "better",
        xy=(2.5, 0.05),
        ...
        arrowprops=dict(
            facecolor="k", width=8, headwidth=30, headlength=30, shrink=0.05
        ),
    )
    cax.annotate(
        "worse",
...
for mc in metric_collections:
    ...
    for jj, met in enumerate(my_metrics):
        ...
        tmp = [
            tab[ii, jj]
            for ii, mod in enumerate(my_models)
            if mod in dict_selection[selection]
        ]
        tmp = NUMPYma__array(tmp).compressed()
    ...
    levels = list(
        range(-2, 3)
    )  # [round(ii, 1) for ii in NUMPYarange(-1.5, 1.6, 0.5)]
    figure_name = OSpath__join(
        path_out, "portraitplot_" + str(mc) + "_modified_v2"
    )
...
if " ":
    ...
    figure_name = OSpath__join(
        path_out,
        "portraitplot_"
        + str(len(metric_collections))
        + "metric_collections_modified_v4",
    )
    tmp_num = ["a) ", "b) ", "c) "]
    title = [
        tmp_num[ii] + dict_mc[mc] + " metric collection"
