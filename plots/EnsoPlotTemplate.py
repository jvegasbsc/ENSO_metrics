...
def my_boxplot(
    ...
    tab_mod, tab_obs, metval, obsname = read_var(
        variables,
        filename_nc,
        model,
        reference,
    ...
    if isinstance(filename_nc, str):
        units = (
            tab_mod[0]
            .units.replace("C", "$^\circ$C")
            .replace("long", "$^\circ$long")
        )
    ...
    if "custom_label" in list(dict_param.keys()):
        ...
        custom_label = None
    nbrl = int(round(nbr_panel / 2.0))
    nbrc = 1 if nbr_panel == 1 else 2
    fig, axes = plt.subplots(
        nbrl, nbrc, figsize=(4 * nbrc, 4 * nbrl), sharex="col", sharey="row"
    )
    legco = ["k", "dodgerblue"]
    ...
    if custom_label is None:
        ...
        tick_labels = list(
            range(int(MATHfloor(mini)), int(MATHceil(maxi)) + 1)
        )
    ...
    for ii in range(nbr_panel):
        ...
        if isinstance(filename_nc, str):
            ...
            boxproperties = {
                ...
                "medianprops": dict(
                    linestyle="-", linewidth=2, color=legco[1]
                ),
                "whiskerprops": dict(
                    linestyle="-", linewidth=2, color=legco[1]
                ),
            ...
            for kk in range(len(tab_mod)):
                boxproperties = {
                    "boxprops": dict(
                        linestyle="-", linewidth=2, color=legco[kk + 1]
                    ),
                    "capprops": dict(
                        linestyle="-", linewidth=2, color=legco[kk + 1]
                    ),
...
def my_curve(
    ...
    if isinstance(filename_nc, str):
        axis = list(
            NUMPYarray(tab_mod[0].coords[list(tab_mod[0].coords.keys())[0]])
        )
    elif isinstance(filename_nc, dict) is True and shading is True:
        axis = list(
            NUMPYarray(
                tab_mod[0][0][0].coords[
                    list(tab_mod[0][0][0].coords.keys())[0]
                ]
            )
        )
    else:
        axis = list(
            NUMPYarray(
                tab_mod[0][0].coords[list(tab_mod[0][0].coords.keys())[0]]
            )
        )
    # figure initialization
    title = dict_param["title"]
    ...
    if isinstance(filename_nc, str):
        ...
        if xname == "months" and nbr == 72:
            ...
            nbrc = 1
            fig, axes = plt.subplots(
                nbrl, nbrc, figsize=(8, 4 * nbrl), sharex="col", sharey="row"
            )
        else:
            nbrl = int(round(nbr_val / 2.0))
        ...
        for kk in range(len(tab_obs)):
            if isinstance(filename_nc, dict) is True and shading is True:
                tab_tmp = [
                    [tab_mod[jj][ll][kk] for ll in range(len(tab_mod[jj]))]
                    for jj in range(len(tab_mod))
                ]
            else:
...
def my_dotplot(
    ...
    # get data
    diag_mod, diag_obs, metval, obsname = read_diag(
        diagnostic_values, metric_values, model, reference, metric_variables
    )
    if metric_type is not None:
        plot_metric = True
    else:
    ...
    if isinstance(filename_nc, str):
        ...
        ax.legend(
            lines, legend, bbox_to_anchor=(1, 1), loc="upper left", ncol=1
        )
    else:
        # legend
        lines = [
...
def my_dot_to_box(
    ...
    if "colors" in list(dict_param.keys()):
        ...
        mcolors = ["k", "dodgerblue"]
        if (
            isinstance(filename_nc, list) is True
            or isinstance(filename_nc, dict) is True
        ):
            for ii in range(len(filename_nc) - 1):
                mcolors.append(colors_sup[ii])
    if "legend" in list(dict_param.keys()):
    ...
    tmp = (
        [diag_obs]
        + [min(my_mask(tt, remove_masked=True)) for tt in tab]
    ...
    for ii in range(len(tab)):
        ...
        boxproperties = {
            "boxprops": dict(
                linestyle="-", linewidth=2, color=mcolors[ii + 1]
            ),
            "capprops": dict(
                linestyle="-", linewidth=2, color=mcolors[ii + 1]
            ),
            "flierprops": dict(
                marker="o",
                markersize=2.0,
                markeredgecolor=mcolors[ii + 1],
                markerfacecolor=mcolors[ii + 1],
                markeredgewidth=0,
            ),
            "meanprops": dict(
                marker="D",
        ...
        tmp = (
            [[1e20, 1e20]] * ii
            + [my_mask(tab[ii], remove_masked=True)]
            + [[1e20, 1e20]] * (len(tab) - 1 - ii)
        )
        ax.boxplot(
            tmp,
            whis=[5, 95],
...
def my_hovmoeller(
    ...
    if isinstance(filename_nc, str):
        ...
        tim = list(
            NUMPYarray(tab_mod[0][0][0].coords[tab_mod[0][0][0].dims[0]])
        )
        lon = list(
            NUMPYarray(tab_mod[0][0][0].coords[tab_mod[0][0][0].dims[1]])
        )
    else:
        tim = list(NUMPYarray(tab_mod[0][0].coords[tab_mod[0][0].dims[0]]))
        lon = list(NUMPYarray(tab_mod[0][0].coords[tab_mod[0][0].dims[1]]))
    nbr_years = len(tim) / 12.0
    # figure initialization
    nbr_panel = dict_param["nbr_panel"]
    if isinstance(filename_nc, list) or (
        isinstance(filename_nc, dict) is True and shading is True
    ):
    ...
    if isinstance(filename_nc, list):
        ...
        title = [obsname] + [
            mod.upper() + " (" + str(len(models2[mod])) + ")" for mod in model
        ]
        if nbr_val > 1:
            title = title * nbr_val
    xname = dict_param["xname"]
    ...
    xlabel_ticks = list(
        range(int(MATHfloor(min(lon))), int(MATHceil(max(lon))) + 1)
    )
    xlabel_ticks, xlabel = create_labels(xname, xlabel_ticks)
    ylabel_ticks = list(
        range(int(MATHfloor(min(tim))), int(MATHceil(max(tim))) + 1)
    )
    ...
    for ii in range(nbr_panel):
        ...
        # title
        if isinstance(filename_nc, str):
            ax.set_title(title[ii / 2], fontsize=15, y=1.0 + hspa2, loc="left")
            if ii in [0, 1]:
                ax.text(
                    0.5,
                    1.0 + hspa1,
            ...
            if nbrc == 2:
                if ii % nbrc == 0:
                    ax.set_title(
                        title[ii], fontsize=15, y=1.0 + hspa2, loc="left"
                    )
                else:
                    ax.set_title(
                        title[ii], fontsize=15, y=1.0 + hspa2, loc="right"
                    )
            else:
                ax.set_title(
                    title[ii], fontsize=15, y=1.0 + hspa2, loc="center"
                )
            if nbr_val > 1:
                if nbrc == 2:
        ...
        cs = ax.contourf(
            xx, yy, tab[ii], levels=levels, extend="both", cmap=colorbar
        )
        if ii == nbr_panel - nbrc:
...
def my_map(
    ...
    if isinstance(variables, str):
        ...
        nbr_val = len(variables)
    if (
        "africaSE" in variables
        or "america" in variables
        or "asiaS" in variables
        or "oceania" in variables
    ):
        met_in_file = True
        my_reg = (
            "africaSE"
            if "africaSE" in variables
            else (
                "americaN"
                if "americaN" in variables
                else (
                    "americaS"
                    if "americaS" in variables
                    else ("asiaS" if "asiaS" in variables else "oceania")
                )
            )
        )
    else:
    ...
    if isinstance(filename_nc, list) is True:
        ...
        tmp_let = ["b) ", "c) ", "d) "]
        title = ["a) ref: " + obsname] + [
            tmp_let[ii] + mod.upper() + " (" + str(len(models2[mod])) + ")"
    ...
    xlabel_ticks, xlabel = create_labels(xname, xlabel_ticks)
    ylabel_ticks = list(
        range(int(MATHfloor(min(lat))), int(MATHceil(max(lat))) + 1)
    )
    ylabel_ticks, ylabel = create_labels(yname, ylabel_ticks)
    tab = list()
    if isinstance(filename_nc, str):
        ...
        for ii in range(len(tab_obs)):
            ...
            for kk in range(len(tab_mod)):
                if (
                    ...
                    tmp = [
                        tab_mod[kk][jj][ii] for jj in range(len(tab_mod[kk]))
                    ]
                tab.append(my_average(tmp, axis=0))
    else:
        legend = ["ref: " + obsname] + model
    ...
    for ii in range(nbr_panel):
        if nbr_panel == 1:
            ...
            ax = axes[ii / nbrc, ii % nbrc]
        if isinstance(filename_nc, str):
            ax.set_title(title[ii / 2], fontsize=15, y=1.0, loc="left")
            if ii in [0, 1]:
                ax.text(
                    0.5,
                    1.0 + (0.15 * (len(lon) + 10)) / len(lat),
                    legend[ii % 2],
                    fontsize=15,
                    weight="bold",
                    horizontalalignment="center",
            ...
            if nbrc == 2 or plot_metric is True:
                if ii == 0 or (ii % nbr_mod == 0):
                    ...
                    location = "left"  # "right"
                ax.set_title(
                    title[ii], fontsize=15, y=1.0 + hspa2, loc=location
                )
                del location
            else:
                if (
        ...
        if ii > 0 and plot_metric is True:
            if isinstance(metric_type, str):
                txt = format_metric(
                    metric_type,
                    my_average(metval[ii - 1], remove_masked=True),
                    metric_units,
                )
                ax.text(
                    0.5,
                ...
                for jj in range(len(metric_type)):
                    if shading is True:
                        tmp = [
                            metval[ii - 1][kk][jj]
                            for kk in range(len(metval[ii - 1]))
                        ]
                    ...
                    if my_reg in [
                        ...
                        "reg_pr_over_sst_map" in variables
                        or "reg_slp_over_sst_map" in variables
                        or "reg_ts_over_sst_map" in variables
                    ):
    ...
    if my_reg in ["africaSE", "americaN", "americaS", "asiaS", "oceania"]:
        ...
        cax = plt.axes(
            [x1, y1 + 0.2, x2 - x1, 0.015]
        )  # cax = plt.axes([x1, y1 + 0.21, x2 - x1, 0.015])
    elif nbrl == 3:
        if (
            article_fig is True
            and nbrl == 3
            and "reg_pr_over_sst_map" in variables
        ):
...
def my_scatterplot(
    ...
    for ii in range(nbr_panel):
        ...
        if "EnsoFbSshSst" in figure_name and article_fig is True:
            ...
            "EnsoFbSstTaux" in figure_name or "EnsoFbSstSwr" in figure_name
        ) and article_fig is True:
        ...
        if (
            one_xaxis is True and (ii >= (nbrc * nbrl) - nbrc)
        ) or one_xaxis is False:
            xlabel = xname[ii]
            for kk in list(regions.keys()):
        ...
        y1, y2 = ax.get_ylim()
        dy = (y2 - y1) / 100.0
        if (nbr_panel > 1 and nbr_panel == nbr_val / 2.0) or (
            nbr_panel == len(legend)
        ):
            # multiple panel
            ...
            for jj in range(len(keys1)):
                if isinstance(filename_nc, dict):
                    ...
                    if ii == 0:
                        if "slope" + keys1[jj] in list(
                            tab_obs[0].attrs.keys()
                        ) and "intercept" + keys1[jj] in list(
                            tab_obs[0].attrs.keys()
                        ):
                            intercept.append(
                                tab_obs[0].attrs["intercept" + keys1[jj]]
                            )
                            slope.append(tab_obs[0].attrs["slope" + keys1[jj]])
                    else:
                        for kk in range(len(tab_mod[ii - 1])):
                            if "slope" + keys1[jj] in list(
                                tab_mod[ii - 1][kk][0].attrs.keys()
                            ) and "intercept" + keys1[jj] in list(
                                tab_mod[ii - 1][kk][0].attrs.keys()
                            ):
                                intercept.append(
                                    tab_mod[ii - 1][kk][0].attrs[
                                        "intercept" + keys1[jj]
                                    ]
                                )
                                slope.append(
                    ...
                    if len(intercept) == 1:
                        ...
                        intercept = float(
                            my_average(intercept, remove_masked=True)
                        )
            ...
            for jj in range(len(tab1)):
                ...
                if isinstance(filename_nc, dict):
                    intercept, slope = list(), list()
                    if jj == len(tab1) - 1:
                        if "slope" + keys1[0] in list(
                            tab_obs[0].attrs.keys()
                        ) and "intercept" + keys1[0] in list(
                            tab_obs[0].attrs.keys()
                        ):
                            intercept.append(
                                tab_obs[0].attrs["intercept" + keys1[0]]
                            )
                            slope.append(tab_obs[0].attrs["slope" + keys1[0]])
                    else:
                        for kk in range(len(tab_mod[len(tab_mod) - 1 - jj])):
                            if "slope" + keys1[0] in list(
                                tab_mod[len(tab_mod) - 1 - jj][kk][
                                    0
                                ].attrs.keys()
                            ) and "intercept" + keys1[0] in list(
                                tab_mod[len(tab_mod) - 1 - jj][kk][
                    ...
                    if "slope" in list(
                        tab1[jj].attrs.keys()
                    ) and "intercept" in list(tab1[jj].attrs.keys()):
                        ...
                        ax.plot(xx, yy, lw=2, c=col)
                        tmp.append(
                            "slope = " + "{0:+.2f}".format(round(slope, 2))
                        )
                ...
                if metric_type is not None:
                    for jj in range(1, len(legend)):
                        if isinstance(model, str):
                            ...
                            tmp = my_average(
                                metval[jj - 1], remove_masked=True
                            )
                        else:
                            tmp = metval[jj - 1]
                        ...
                        if (
                            "EnsoFbSshSst" in figure_name
                            or "EnsoFbSstTaux" in figure_name
                            or "EnsoFbTauxSsh" in figure_name
                        ) and article_fig is True:
                            metric_units = "%"
                            legend[jj] = (
                                legend[jj]
                                + " ("
                                + "{0:}".format(int(round(tmp)))
                                + " "
                                + metric_units
                                + ")"
                            )
...
def plot_curve(
    ...
    # x axis
    label_ticks = list(
        range(int(MATHfloor(min(axis))), int(MATHceil(max(axis))) + 1)
    )
    ...
    if multimodel is False:
        ...
        if len(linecolors["model"]) == 1 and len(linecolors["reference"]) == 1:
            ...
            ax.legend(
                lines, legtxt, bbox_to_anchor=(1, 1), loc="upper left", ncol=1
            )
        # my text
        if plot_metric is True:
            txt = format_metric(metric_type, metval, metric_units)
