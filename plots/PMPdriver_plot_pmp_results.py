...
"""
path_in_js = (
    "/work/lee1043/imsi/result_test/metrics_results/enso_metric/cmip5/historical/v20191204/"
    + metric_collection
)
path_in_nc = (
    "/work/lee1043/imsi/result_test/diagnostic_results/enso_metric/cmip5/historical/v20191204/"
    + metric_collection
)
path_out = (
    "/work/lee1043/imsi/result_test/graphics/enso_metric/cmip5/historical/v20191204/"
    + metric_collection
)
...
pattern = "_".join(
    [project, experiment, metric_collection, "v????????", model, member]
)
...
for met in metrics:
    ...
    filename_nc = list(
        GLOBiglob(OSpath__join(path_nc, pattern + "_" + met + ".nc"))
    )
    if len(filename_nc) != 1:
        ...
        if metric_collection == "ENSO_tel" and "Map" in met:
            ...
            diagnostic_values = dict(
                (key1, None) for key1 in list(dict_dia.keys())
            )
            diagnostic_units = ""
        else:
            dict_dia = data_json["value"][met]["diagnostic"]
            diagnostic_values = dict(
                (key1, dict_dia[key1]["value"])
                for key1 in list(dict_dia.keys())
            )
        ...
        if metric_collection == "ENSO_tel" and "Map" in met:
            ...
            metric_values = dict(
                (key1, {model: dict_met[key1]["value"]})
                for key1 in list(dict_met.keys())
            )
