    ...
    if plot_legend is True:
        ...
        lines = [
            Line2D(
                ...
                marker=martmp[kk],
                c="w",
                markerfacecolor=coltmp[kk],
                markersize=msitmp[kk],
            )
...
def transition(text, yy=0.6, ind=0, nint=2, uppertitle=""):
    ...
    ax.text(
        0.5, 1.3, uppertitle, fontsize=30, transform=ax.transAxes, **fondict
    )
    return
...
for proj in list_project:
    ...
    for mc in metric_collection:
        ...
        # units
        if proj == "cmip5":
            dicttmp = data["RESULTS"]["model"]["CNRM-CM5"]["metadata"][
                "metrics"
            ]
            keystmp = list(dicttmp.keys())
            for met in keystmp:
                uni = (
                    dicttmp[met]["metric"]["units"]
                    if ("Rmse" in met or "Corr" in met)
                    else dicttmp[met]["diagnostic"]["units"]
                )
                dict_uni[met] = uni.replace("C", "$^\circ$C").replace(
                    "long", "$^\circ$long"
                )
            del dicttmp, keystmp
        list_models = sorted(
            list(data["RESULTS"]["model"].keys()), key=lambda v: v.upper()
        )
        # read metrics
        ...
        for mod in list_models:
            ...
            for met in list_metrics:
                if proj == "cmip5":
                    dicttmp1 = data["RESULTS"]["model"]["CNRM-CM5"]["value"][
                        met
                    ]["metric"]
                else:
                    dicttmp1 = data["RESULTS"]["model"]["CNRM-CM6-1"]["value"][
                        met
                    ]["metric"]
                keystmp = list(dicttmp1.keys())
                # metric
                try:
                    data["RESULTS"]["model"][mod]["value"][met]["metric"]
                except:
                    tmp = dict((key1, 1e20) for key1 in keystmp)
                else:
                    dicttmp2 = data["RESULTS"]["model"][mod]["value"][met][
                        "metric"
                    ]
                    tmp = dict(
                        (
                            ...
                            1e20
                            if (
                                key1 not in list(dicttmp2.keys())
                                or dicttmp2[key1]["value"] is None
                            )
                            else dicttmp2[key1]["value"],
                        )
                        for key1 in keystmp
                    )
                dict21[met] = tmp
                ...
                try:
                    data["RESULTS"]["model"][mod]["value"][met]["diagnostic"]
                except:
                    tmp = dict(
                        (
                            key1,
                            1e20 if key1 == mod else dicttmp1[key1]["value"],
                        )
                    ...
                    tmp = dict(
                        (
                            ...
                            if (
                                ...
                                1e20
                                if dicttmp2[key1]["value"] is None
                                else dicttmp2[key1]["value"]
                            ),
                        )
                        for key1 in keystmp + [mod]
...
if big_ensemble is True:
    ...
    for key1 in lev1:
        if len(list(dict_met[key1].keys())) == max(list1):
            list_metrics = sorted(
                list(dict_met[key1].keys()), key=lambda v: v.upper()
            )
            pass
    # !!!!! temporary: start !!!!!
    ...
    for key2 in list_metrics:
        for key1 in lev1:
            if key2 not in list(dict_met[key1].keys()):
                dict_met[key1][key2] = dict(
                    (ref, 1e20)
                    for ref in list(dict_met["ACCESS1-0"][key2].keys())
                )
    list1 = list(set([len(list(dict_met[key1].keys())) for key1 in lev1]))
    # !!!!! temporary: end !!!!!
    # !!!!! temporary: start !!!!!
    ...
    for key2 in list_metrics:
        for key1 in lev1:
            if key2 not in list(dict_dia[key1].keys()):
                dict_dia[key1][key2] = dict(
                    (ref, 1e20)
                    for ref in list(dict_dia["ACCESS1-0"][key2].keys())
                )
    # !!!!! temporary: end !!!!!
else:
    list1 = list(
        set(
            [
                len(list(dict_met[key1][key2].keys()))
                for key1 in lev1
    ...
    for key1 in lev1:
        for key2 in list(dict_met[key1].keys()):
            if len(list(dict_met[key1][key2].keys())) == max(list1):
                list_metrics = sorted(
                    list(dict_met[key1][key2].keys()), key=lambda v: v.upper()
                )
                pass
    # !!!!! temporary: start !!!!!
    # some models are not used in all metric collection (ask jiwoo)
            ...
            for key2 in list(dict_met[key1].keys()):
                if key3 not in list(dict_met[key1][key2].keys()):
                    if isinstance(dict_met["cmip5"]["ACCESS1-0"][key3], dict):
                        dict_met[key1][key2][key3] = dict(
                            (ref, 1e20)
                            for ref in list(
                                dict_met["cmip5"]["ACCESS1-0"][key3].keys()
                            )
                        )
                    else:
...
if " ":
    ...
    mimadict = {
        ...
        "BiasPrLonRmse": [0, 3],
        "BiasSstLonRmse": [0, 3],
        "EnsoSstLonRmse": [0.0, 0.6],
        "EnsoAmpl": [0.4, 1.6],
        "EnsodSstOce": [0, 3],
        "EnsoSeasonality": [0.5, 2.5],
        "EnsoSstSkew": [-0.8, 0.8],
        "NinoSstDiversity": [0, 70],
        "EnsoFbSshSst": [0.1, 0.3],
        "EnsoFbSstTaux": [0, 15],
        "EnsoFbSstThf": [-25, 5],
        "EnsoFbTauxSsh": [0.1, 0.4],
        "EnsoPrMapRmse": [0.00, 0.35],
        "EnsoPrMapCorr": [0, 1],
    }
    # my_col = ["dodgerblue", "k"]
    ...
    numtmp = ["a) ", "b) ", "c) "]
    nbr = sum(
        [5 if "Rmse" in met else 9 for ii, met in enumerate(list1)]
    ) + 3 * len(list1)
    ...
    for ii, met in enumerate(list1):
        ...
        if "Rmse" not in met:
            if "Corr" in met:
                ...
                my_text = "1 - corr"
                val = [
                    list2[ii][mod][met][ref]
                    for mod in list(list2[ii].keys())
                    if list2[ii][mod][met][ref] < 1e15
                ]
                obs = 1
            else:
                tname = "scalar"
                ...
                val = [
                    list2[ii][mod][met][mod]
                    for mod in list(list2[ii].keys())
                    if list2[ii][mod][met][mod] < 1e15
                ]
                obs = list2[ii][list(list2[ii].keys())[0]][met][ref]
        ...
        if "Rmse" in met:
            ...
            xlabel = "% of error"
            val = [abs(100.0 * (jj - obs) / obs) for jj in val]
            my_ylim = [0, 80]
