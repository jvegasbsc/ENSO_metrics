...
models = ["IPSL-CM6A-LR"] * 32 + [
    "IPSL-CM5A-LR"
] * 6  # ["CNRM-CM5", "CNRM-CM6-1", "CNRM-CM6-1"]# "CNRM-ESM2-1", "IPSL-CM6A-LR"]
experiment = "historical"  # "piControl" #
members = ["r" + str(ii + 1) + "i1p1f1" for ii in range(32)] + [
    "r" + str(ii + 1) + "i1p1" for ii in range(6)
...
for met in metrics:
    ...
    for ii in range(len(models)):
        tmp = (
            pattern.replace("modelname", models[ii]).replace(
                "membername", members[ii]
            )
            + "_"
            + met
            + ".nc"
    ...
    diagnostic_values = dict(
        (key1, data_json["value"][met]["diagnostic"][key1]["value"])
    ...
    metric_values = dict(
        (
            key1,
            dict(
                (key2, data_json["value"][met]["metric"][key1][key2]["value"])
                for key2 in list(
                    data_json["value"][met]["metric"][key1].keys()
                )
            ),
        )
        for key1 in list(data_json["value"][met]["metric"].keys())
    )
    diagnostic_units = data_json["metadata"]["metrics"][met]["diagnostic"][
        "units"
    ]
