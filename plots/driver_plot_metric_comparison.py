...
def bootstrap(
    tab, num_samples=1000000, alpha=0.05, nech=None, statistic=NUMPYmean
):
...
def plot_metrics(
    ...
    ax.set_yticklabels(
        ["reference", xticklabel, "2 * " + xticklabel],
    ...
    for ii in range(len(tab_val)):
        ...
        ax.scatter(
            axis, list(tab_val[ind]), s=200, c=col, marker="D", zorder=2
        )
        if tab_bst is not None:
            for jj in range(len(tab_bst[ind])):
                if tab_bst[ind][jj][0] > 0 and tab_bst[ind][jj][1] > 0:
                    ax.add_line(
                        Line2D(
                            [jj - 0.3, jj + 0.3],
                            [tab_bst[ind][jj][0], tab_bst[ind][jj][0]],
                            c=col,
                            lw=2,
                            zorder=3,
                        )
                    ...
                    ax.add_line(
                        Line2D(
                            [jj - 0.3, jj + 0.3],
                            [tab_bst[ind][jj][1], tab_bst[ind][jj][1]],
                            c=col,
                            lw=2,
                            zorder=3,
                        )
                    ...
                    ax.add_line(
                        Line2D(
                            [jj, jj],
                            [tab_bst[ind][jj][0], tab_bst[ind][jj][1]],
                            c=col,
                            lw=2,
                            zorder=3,
                        )
    ...
    if legend is not None:
        ...
        ax.add_line(
            Line2D([xxx, xxx], [yyy - ddy, yyy + ddy], c=colors[1], lw=2)
        )
        dicttext = {
        ...
        ax.text(
            xxx + dx,
            yyy,
            "95% confidence interval of MMM\n(Monte Carlo sampling method)",
            **dicttext
        )
        arrowdict = dict(
            facecolor="k", width=2, headwidth=10, headlength=10, shrink=0.0
        )
        ax.annotate(
            "",
            xy=(-0.05, 0.05),
        ...
        for mod in list_models:
            ...
            for met in list_metrics:
                try:
                    ...
                    key1 = list(
                        data["RESULTS"]["model"][mod]["value"][met][
                            "metric"
                        ].keys()
                    )[0]
                    tmp = (
                        1e20
                        if data["RESULTS"]["model"][mod]["value"][met][
                            "metric"
                        ][key1]["value"]
                        is None
                        else data["RESULTS"]["model"][mod]["value"][met][
                            "metric"
                        ][key1]["value"]
                    )
                dict2[met] = tmp
...
if " ":
    if big_ensemble is True:
        ...
        figure_name = OSpath__join(
            path_out,
            "metrics_comparison_"
            + str(len(list_metrics)).zfill(2)
            + "metrics_"
            + str(len(my_project)).zfill(2)
            + "selections_"
            + my_project[0]
            + "",
        )
        ...
        #     del tmp
        list_metrics2 = [
            met.replace("_1", "").replace("_2", "") for met in list_metrics
        ]
        list_metrics2 = [
            "" if met in ["EnsoFbSstTaux", "EnsoFbSstThf"] else met
            for met in list_metrics2
        ]
        list_names2 = [
            str(len(dict_selection[my_project[0]])) + " models",
            my_project[1],
        ]
        plot_metrics(
            tab_val,
