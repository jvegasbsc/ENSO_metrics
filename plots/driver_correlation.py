    ...
    for ii in range(len(tab)):
        ...
        for jj in range(len(tab)):
            ...
            (
                ...
                p_value,
                std_err,
            ) = SCIPYstats__linregress(tmpf1, tmpf2)
...
def get_reference(metric_collection, metric):
    if metric_collection == "ENSO_tel" and "Map" in metric:
        my_met = (
            metric.replace("Corr", "").replace("Rmse", "").replace("Std", "")
        )
    else:
...
def get_ref(metric):
    for mc in metric_collection:
        list_met = sorted(
            list(defCollection(mc)["metrics_list"].keys()),
...
def plot_correlation(
    ...
    **Optional arguments:**
    ...
    if tab_pval is not None:
        mask1 = NUMPYmasked_where(tab_rval < tab_pval, tab_rval).mask.astype(
            "f"
        )
        mask2 = NUMPYmasked_where(tab_rval > -tab_pval, tab_rval).mask.astype(
            "f"
        )
        mask = mask1 + mask2
    ...
    if cfram is True:
        ...
        lix = (
            [[8, 8], [15, 15], [19, 19], [0, nbr], [0, nbr], [0, nbr]]
            + [[0, 0], [nbr, nbr], [0, 8], [0, 8]]
            + [[0, 0], [nbr, nbr], [8.18, 15], [8.18, 15]]
            + [[0, 0], [nbr, nbr], [15.18, 19], [15.18, 19]]
            + [[0, 0], [nbr, nbr], [19.18, nbr], [19.18, nbr]]
        )
        liy = (
            [[0, nbr], [0, nbr], [0, nbr], [8, 8], [15, 15], [19, 19]]
            + [[0, 8], [0, 8], [0, 0], [nbr, nbr]]
            + [[8.18, 15], [8.18, 15], [0, 0], [nbr, nbr]]
            + [[15.18, 19], [15.18, 19], [0, 0], [nbr, nbr]]
            + [[19.18, nbr], [19.18, nbr], [0, 0], [nbr, nbr]]
        )
        ...
        for ii in range(len(tab_plot)):
            nbr1 = str(
                sum(
                    [
                        1
                        for jj in range(len(tab_plot[ii]))
                        if tab_plot[ii][jj] < 0
                    ]
                )
            )
...
for proj in list_project:
    if big_ensemble is not True or (
        big_ensemble is True and proj == list_project[0]
    ):
        dict_mc = dict()
    for mc in metric_collection:
        # read json
        ...
        for mod in list_models:
            ...
            list_metrics = sorted(
                list(data_mod.keys()), key=lambda v: v.upper()
            )
            ...
            for met in list_metrics:
                ...
                list_ref = sorted(
                    list(data_met.keys()), key=lambda v: v.upper()
                )
                my_ref = deepcopy(ref)
                # if ref in list_ref:
                #     my_ref = deepcopy(ref)
...
if " ":
    if big_ensemble is True:
        ...
        lev1 = sorted(list(dict_out.keys()), key=lambda v: v.upper())
        list_metrics = sorted(
            list(dict_out[lev1[0]].keys()), key=lambda v: v.upper()
        )
        ...
        name_plot = OSpath__join(
            path_out,
            "correlations_inter_metrics_"
            + str(len(list_metrics)).zfill(2)
            + "metrics_"
            + str(len(lev1)).zfill(2)
            + "models_v3",
        )
...
if "":
    tmp1 = [
        dict_out[mod]["EnsoSeasonality"] for jj, mod in enumerate(lev1)
    ]  # Q1 ~ 0.25 / Q2 ~ 0.34
    tmp2 = [
        dict_out[mod]["BiasSstLonRmse"] for jj, mod in enumerate(lev1)
    ]  # Q1 ~ 0.64 / Q2 ~ 1.00
    tmp3 = [
        dict_out[mod]["BiasPrLonRmse"] for jj, mod in enumerate(lev1)
    ]  # Q1 ~ 0.73 / Q2 ~ 1.00
    tmp4 = [
        dict_out[mod]["SeasonalSstLonRmse"] for jj, mod in enumerate(lev1)
    ]  # Q1 ~ 0.18 / Q2 ~ 0.21
    tmp5 = [
        dict_out[mod]["SeasonalPrLonRmse"] for jj, mod in enumerate(lev1)
    ]  # Q1 ~ 0.44 / Q2 ~ 0.63
    tmp6 = [
        dict_out[mod]["EnsoFbSstTaux"] for jj, mod in enumerate(lev1)
    ]  # Q1 ~ 0.39 / Q2 ~ 0.47
    tmp7 = [
        dict_out[mod]["EnsoFbSstThf"] for jj, mod in enumerate(lev1)
    ]  # Q1 ~ 0.23 / Q2 ~ 0.48
    # tmp8 = [dict_out[mod]["EnsoFbSstSwr"] for jj, mod in enumerate(lev1)]  # Q1 ~ 0.54 / Q2 ~ 1.18
    tmp9 = [
        dict_out[mod]["EnsodSstOce_2"] for jj, mod in enumerate(lev1)
    ]  # Q1 ~ 0.17 / Q2 ~ 0.33
    tmp10 = [
        dict_out[mod]["EnsoSstSkew"] for jj, mod in enumerate(lev1)
    ]  # Q1 ~ 0.52 / Q2 ~ 0.82
    selections = [
        [
            lev1[ii]
            for ii in range(len(tmp1))
            if tmp1[ii] < 0.25 and tmp2[ii] < 1.0
        ],
        [
            lev1[ii]
            for ii in range(len(tmp1))
            if tmp1[ii] < 0.25 and tmp3[ii] < 1.0
        ],
        [
            lev1[ii]
            for ii in range(len(tmp1))
            if tmp1[ii] < 0.25 and tmp4[ii] < 0.25
        ],
        [
            lev1[ii]
            for ii in range(len(tmp1))
            if tmp1[ii] < 0.25
            and tmp5[ii] < SCIPYstats__scoreatpercentile(tmp5, 50)
        ],
        [
            lev1[ii]
            for ii in range(len(tmp1))
            if tmp1[ii] < 0.25 and tmp6[ii] < 0.5
        ],
        [
            lev1[ii]
            for ii in range(len(tmp1))
            if tmp1[ii] < 0.25 and tmp7[ii] < 0.5
        ],
        # [lev1[ii] for ii in range(len(tmp1)) if tmp1[ii] < 0.25 and tmp8[ii] < 1.0],
        [
            lev1[ii]
            for ii in range(len(tmp1))
            if tmp6[ii] < SCIPYstats__scoreatpercentile(tmp6, 25)
            and tmp7[ii] < SCIPYstats__scoreatpercentile(tmp7, 25)
        ],
        [
            lev1[ii]
            for ii in range(len(tmp1))
            if tmp6[ii] < SCIPYstats__scoreatpercentile(tmp6, 50)
            and tmp9[ii] < SCIPYstats__scoreatpercentile(tmp9, 25)
        ],
        [
            lev1[ii]
            for ii in range(len(tmp1))
            if tmp7[ii] < SCIPYstats__scoreatpercentile(tmp7, 50)
            and tmp9[ii] < SCIPYstats__scoreatpercentile(tmp9, 25)
        ],
        [
            lev1[ii]
            for ii in range(len(tmp1))
            if tmp1[ii] < SCIPYstats__scoreatpercentile(tmp1, 50)
            and tmp10[ii] < SCIPYstats__scoreatpercentile(tmp10, 50)
        ],
    ]
