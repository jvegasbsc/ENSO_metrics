...
def EnsoAmpl(
    ...
    **kwargs
    ...
    sst, sst_areacell, keyerror = read_data(
        sstfile,
        sstname,
        "temperature",
        metric,
        sstbox,
        file_area=sstareafile,
        name_area=sstareaname,
        file_mask=sstlandmaskfile,
        name_mask=sstlandmaskname,
        mask_land=True,
        mask_ocean=False,
        debug=debug,
        **kwargs
    ...
    if keyerror is not None:
        ...
        sst, Method = pre_process_data(
            sst,
            Method,
            areacell=sst_areacell,
            average="horizontal",
        ...
        if debug is True:
            ...
            EnsoErrorsWarnings.debug_mode(
                "\033[92m", "after PreProcessTS", 15, **dict_debug
            )
        ...
        if netcdf is True:
            ...
            # Read file and select the right region
            sst1, sst_areacell1, unneeded = read_data(
                sstfile,
                sstname,
                "temperature",
                metric,
            ...
            sst2, sst_areacell2, unneeded = read_data(
                sstfile,
                sstname,
                "temperature",
                metric,
            ...
            # Preprocess variables (computes anomalies, normalizes, detrends TS, smoothes TS, averages horizontally)
            sst1, unneeded = pre_process_data(
                sst1,
                "",
                areacell=sst_areacell1,
                interannual_anomalies=True,
                region="equatorial_pacific_LatExt2",
                **kwargs
            )
            sst2, unneeded = pre_process_data(
                sst2,
                "",
                areacell=sst_areacell2,
                interannual_anomalies=True,
            ...
            if debug is True:
                dict_debug = {
                    "axes1": "(sst1) "
                    + str([ax.id for ax in sst1.getAxisList()]),
                    "axes2": "(sst2) "
                    + str([ax.id for ax in sst2.getAxisList()]),
                    "shape1": "(sst1) " + str(sst1.shape),
                    "shape2": "(sst2) " + str(sst2.shape),
                    "time1": "(sst1) " + str(get_time_bounds(sst1)),
                    "time2": "(sst2) " + str(get_time_bounds(sst2)),
                }
                EnsoErrorsWarnings.debug_mode(
                    "\033[92m", "after PreProcessTS", 10, **dict_debug
                )
            # std
            ...
            sst2 = compute_regrid(
                sst2, None, region="equatorial_pacific", **kwargs["regridding"]
            )
            # Meridional average
            sst2 = average_meridional(sst2)
            # Dive down diagnostic
            if ".nc" in netcdf_name:
                file_name = deepcopy(netcdf_name).replace(
                    ".nc", "_" + metname + ".nc"
                )
            else:
            ...
            save_netcdf(
                file_name,
                var1=sst2,
                var1_attributes=dict1,
                var1_name="sstStd_lon__" + dataset,
                var2=sst1,
                var2_attributes=dict2,
                var2_name="sstStd_map__" + dataset,
    ...
    if debug is True:
        dict_debug = {
            "line1": "metric value: " + str(sstStd),
            "line2": "metric value_error: " + str(sstStdErr),
        }
        EnsoErrorsWarnings.debug_mode(
            "\033[92m", "end of " + metric, 10, **dict_debug
        )
    ...
    amplMetric = {
        "name": Name,
        "value": sstStd,
        "value_error": sstStdErr,
        "units": Units,
        "method": Method,
        "nyears": yearN,
        "time_frequency": kwargs["frequency"],
        "time_period": actualtimebounds,
        "ref": Ref,
        "keyerror": keyerror,
    }
    return amplMetric
